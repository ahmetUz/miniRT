...
/*   Updated: 2022/12/28 16:40:41 by ple-lez          ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */

#include "miniRT_bonus.h"

static int	ft_check_cone(char **args, int line_count)
{
	...
	if (!ft_valid_texture(args[8]))
		return (ft_perturbation_error(line_count, args[8]));
	return (EXIT_SUCCESS);
}
...
int	ft_parse_cone(t_env *env, char **args, int line_count)
{
	...
	if (ft_check_cone(args, line_count) != EXIT_SUCCESS)
		return (EXIT_FAILURE);
